...
function projection(   dataobject::HydroDataType, vars::Array{Symbol,1};
    ...
    if direction == :z
        ...
        newrange1 = range(r1, stop=r2-1, length=(r2-r1)+1)
        newrange2 = range(r3, stop=r4-1, length=(r4-r3)+1)
        #println(newrange1)
        #println(newrange2)
    ...
    if notonly_ranglecheck_vars
        @showprogress 1 "" for level = lmin:simlmax
            ...
            if direction == :z
                # range on maximum used grid
                new_level_range1 = range(rl1, stop=rl2-1, length=(rl2-rl1) +1)
                new_level_range2 = range(rl3, stop=rl4-1, length=(rl4-rl3) +1)
                println(newrange1)
                println(newrange2)
                ...
                for ivar in selected_vars
                    ...
                    if !in(ivar, rσanglecheck)  # exclude velocity dispersion symbols and radius/angle maps
                        ...
                        if level != simlmax
                            ...
                            #scaled_length1 = range(r1, stop=r2-1, length=(r2-r1) +1) #Int(ratio_level * (length_level1-alt_shift) )
                            #scaled_length2 = range(r3, stop=r4-1, length=(r4-r3) +1) #Int(ratio_level * (length_level2-alt_shift) )

                            # map on lmax grid
                            #println(scaled_length1, " ", scaled_length2)
                            remapped = [map_buffer[floor(Int,x),floor(Int,y)]  for x in range(1, stop=s[1], length=length1) , y in range(1, stop=s[2], length=length2)]

                            if !in(ivar, density_names) && !in(ivar, sd_names) && weighting == :mass
                            #if ivar != :sd && ivar != :rho && weighting == true

                                if first_time_level[counter] == 1
                                    map[:, :, counter] .+= remapped ./ (2^simlmax / 2^level  )^2
                                    remap_weight = [map_buffer_weight[floor(Int,x),floor(Int,y)]  for x in range(1, stop=s[1], length=length1), y in range(1, stop=s[2], length=length2)]
                                    map_weight[:, :, counter] .+= remap_weight ./ (2^simlmax / 2^level  )^2  # and correct for remapping
                                    first_time_level[counter] = 0
                                end
                            elseif in(ivar, density_names)
                                #if first_time_level[counter] == 1
                                map[:, :, counter] .+= remapped
                                remap_weight = [map_buffer_weight[floor(Int,x),floor(Int,y)]  for x in range(1, stop=s[1], length=length1), y in range(1, stop=s[2], length=length2)]
                                map_weight[:, :, counter] .+= remap_weight
                                #first_time_level[counter] = 0
                                #end

                            else
                                test = remapped #[Int(1-r1_diff):Int(end-r2_diff), Int(1-r3_diff):Int(end-r4_diff)]
                                println(size(test))
                                println(size(map[:, :, counter] ))
