...
!!! tip "Run it yourself"
    This page is also an executable **Jupyter notebook** — [open / download `14_multi_OffAxis_Features.ipynb`](https://github.com/ManuelBehrendt/Notebooks/blob/master/Mera-Docs/version_1/14_multi_OffAxis_Features.ipynb). The notebooks run end-to-end and double as part of Mera's test suite.
...
gas  = gethydro(getinfo(100, joinpath(BASE, "RAMSES/spiral_clumps"), verbose=false), verbose=false, show_progress=false);
```
...
![](14_multi_OffAxis_Features_files/14_multi_OffAxis_Features_7_1.png)
...
sf = slice(gas, :rho, :nH; direction=:faceon, win..., verbose=false)
se = slice(gas, :rho, :nH; direction=:edgeon, win..., verbose=false)
si = slice(gas, :rho, :nH; inclination=60, azimuth=30, axis=:angmom, win..., verbose=false)

fig = Figure(size=(1500,440))
for (k,(s,t)) in enumerate(((sf,"face-on (midplane)"),(se,"edge-on (vertical cut)"),(si,"inclined 60 deg")))
    showmap!(fig, (1,2k-1), s.map, s.extent .* gas.scale.kpc; title="$t  nH", clabel="log10 n_H [cm^-3]")
end
fig
```

![](14_multi_OffAxis_Features_files/14_multi_OffAxis_Features_9_1.png)
...
```julia
frames = rotation_sequence(gas, :sd, :Msol_pc2; sweep=:azimuth, angles=0:60:300,
                           inclination=55, axis=:angmom, center=[:bc], pxsize=[0.35,:kpc],
                           aperture=:square, verbose=false)   # fov omitted → auto-fit the whole galaxy

# aperture=:square + auto fov: the full galaxy in a full rectangular frame, fixed scale at every
# angle (every frame identical size). Set fov=… (e.g. 16, fov_unit=:kpc) to zoom in instead.
fig = Figure(size=(1500,280))
for (k,f) in enumerate(frames)
    ax = Axis(fig[1,k], aspect=DataAspect(), title="azimuth $((k-1)*60) deg"); hidedecorations!(ax)
    heatmap!(ax, log10.(map(v-> v>0 ? Float64(v) : NaN, f.maps[:sd])); colormap=:inferno, nan_color=:black)
end
fig
```

![](14_multi_OffAxis_Features_files/14_multi_OffAxis_Features_11_1.png)
