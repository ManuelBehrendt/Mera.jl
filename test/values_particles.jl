...
@testset "get extent" begin
    ...
    @test rx,ry,rz = getextent(part, :kpc)
    @test rxu, ryu, rzu = getextent(part, unit=:kpc)
    @test rx == rxu
    @test fry == ryu
    @test rz == rzu

    @test rx,ry,rz = getextent(part, :kpc, center=[:bc])
    ...
    @test rx,ry,rz = getextent(part, :kpc, center=[0.5,0.5,0.5])
    @test rx[1] == -50 atol=1e-10
    @test rx[2] == 50 atol=1e-10
    @test ry[1] == -50 atol=1e-10
    @test ry[2] == 50 atol=1e-10
    ...
    @test rx,ry,rz = getextent(part, :kpc, center=[0.5,0.5,0.5], center_unit=:kpc)
